...
public class SoundManager : MonoBehaviour
{
    ...
    public void PlaySoundAtPosition(SoundScriptableObject sound, Vector3 playPosition, float spatialBlend = 1f) 
    {
        // if there is no sound then just log an error
        if (sound == null) { Debug.LogError("The Sound is null, did you forgot to set it?"); return; }
    ...
    private void SetAudioSourceSettings(SoundScriptableObject sound, AudioSource audioSource)
    {
    ...
    private float GetSoundVolume(SoundScriptableObject sound) => Random.Range(sound.volume.x, sound.volume.y);
    
    // get a random sound pitch based on the sound min and max pitches
    private float GetSoundPitch(SoundScriptableObject sound) => Random.Range(sound.pitch.x, sound.pitch.y);
